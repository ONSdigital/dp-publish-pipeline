...
func uploadFile(zebedeeRoot string, jsonMessage []byte, bucketName string, completeFileProducer, completeFileFlagProducer kafka.Producer) error {
	...
	if !strings.HasSuffix(message.FileLocation, ".json") {
		...
		if decryptErr == nil {
			s3Client.AddObject(string(content), message.FileLocation, message.CollectionId, message.ScheduleId)
			s3Location := "s3://" + bucketName + message.FileLocation
			fileComplete, _ := json.Marshal(kafka.FileCompleteMessage{FileId: message.FileId, ScheduleId: message.ScheduleId, CollectionId: message.CollectionId, FileLocation: message.FileLocation, S3Location: s3Location})
